...
# to make sure we cover total readout time
beating_time = 2 * N_readouts_target * time_per_readout
hearts = []
for temporal_resolution in temporal_resolutions:
    for heart_beat_length in heart_beat_lengths:
        hearts.append(Heart(beating_time, temporal_phase_resolution=temporal_resolution, average_bps=heart_beat_length,
                            bps_rms=0.05, bps_lower_limit=heart_beat_length-0.05, bps_higher_limit=heart_beat_length+0.05))
...
for heart in hearts:
    for number_of_projections_per_interleaf in numbers_of_projections_per_interleaf:
        print('tres {}; heartbeat {}; n proj {}'.format(
            heart.temporal_phase_resolution, heart.average_bps, number_of_projections_per_interleaf))
        n_interleaf = find_best_fibo(
            N_readouts_target, number_of_projections_per_interleaf, fibonacci)
        phyllo = SpiralPhyllotaxisPattern(n_interleaf*number_of_projections_per_interleaf,
                                          n_interleaf, time_per_acquisition=time_per_readout, alternated_points=True)
        phyllo.separate_in_time_phases(heart.beats)
        phyllo.draw(title='N interleaves = {} ; {} readouts per interleaf ; Ntot = {}'.format(phyllo.n_interleaf, phyllo.n_points/phyllo.n_interleaf, phyllo.n_points), alpha=0.3, marker_size=1, colour='cardiac_phase',
                    filter_func=only_first_cardiac_phase, display=True)  # save='pres_18_05_2020/tres{}_beat{}_proj{}.png'.format(heart.temporal_phase_resolution,heart.average_bps,number_of_projections_per_interleaf)
