...
heart = Heart(N_target*time_per_acquisition * 2)
...
for fibo in fibonacci:
    for add_readout_ends in [True, False]:
        ...
        test.separate_in_time_phases(heart.phases)
        test.draw(title='N_total = {} , {} interleaf'.format(N*fibo,fibo), alpha=0.3, marker_size=1, colour='cardiac_phase', filter_func=only_first_cardiac_phase, save='pres_15_05_2020\{}interleaves_with{}_ends.png'.format(fibo,'' if add_readout_ends else 'out'))